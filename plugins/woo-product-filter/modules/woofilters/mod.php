...
class WoofiltersWpf extends ModuleWpf {
	...
	public function forceElementorProductFilter() {
		if ( '' !== $this->mainWCQueryFiltered ) {
			$this->mainWCQueryFiltered['paged'] = get_query_var( 'paged' );
			$GLOBALS['wp_query']                = new WP_Query( $this->mainWCQueryFiltered );
		}
	}
	...
	public function addCustomTaxQuery( $taxQuery, $data, $mode ) {
		...
		foreach ($data as $key => $param) {
			if ( is_string( $param ) ) {
				...
				} else if ( strpos( $key, 'wpf_filter_cat_' ) !== false || ( 'filter_cat' == $key ) ) {
					if ( ! empty( $param ) ) {
						$idsAnd     = explode( ',', $param );
						$idsOr      = explode( '|', $param );
						$isAnd      = count( $idsAnd ) > count( $idsOr );
				...
				} else if ( strpos( $key, 'wpf_filter_pwb_list' ) !== false ) {
					if ( ! empty( $param ) ) {
						$idsAnd     = explode( ',', $param );
						$idsOr      = explode( '|', $param );
						$isAnd      = count( $idsAnd ) > count( $idsOr );
				...
				} else if ( strpos( $key, 'wpf_filter_' ) === 0 ) {
					if ( ! empty( $param ) ) {
						$idsAnd    = explode( ',', $param );
						$idsOr     = explode( '|', $param );
						$isAnd     = count( $idsAnd ) > count( $idsOr );
			...
			if ( $settingsFilteringByVariations && ! isset( $args['variations'] ) ) {
				...
				if (!empty($clauses['join'])) {
					...
					if ( $metaKeyId ) {
						...
						$sqlOutofstock = ( get_option( 'woocommerce_hide_out_of_stock_items' ) === 'yes' )
							? ' AND md' . $i . '.val_id!=' . $modelMetaValues->getMetaValueId( $metaKeyId, 'outofstock' )
							: '';
			...
			switch ( $taxonomy ) {
				...
				default:
					if ( strpos( $taxonomy, 'pa_' ) === 0 ) {
						$blocks[ $taxonomy ][] = 'wpf_filter_' . substr( $taxonomy, 3 );
					} else {
						$blocks[ $taxonomy ][] = 'wpf_filter_' . $taxonomy;
					}
					break;
			}
		}
